...
import {
  ...
  cartSubtotal, cartDelivery, cartTotalEstimate, freeDeliveryThreshold, minOrderAmount,
} from "../store.js";
...
const canCheckout = computed(() => store.cart.length > 0 && !belowMin.value && !(store.quoteError && store.quoteError.code === "item_unavailable") && !store.catalogClosed);
